...
- **限流 / 防爆破**：应用**已内置按用户的业务频率限制**——后台「安全」面板开启「频率限制」并设阈值后，发动态（每分 / 每时）、发帖（每分）、私信（每分）超限即返回 429（管理员豁免；Redis 计数；故障 fail-open）。注意它是**按登录用户**计数，不覆盖**登录爆破、批量注册、CC/DDoS** 这类按 IP / 未登录的攻击——这些仍建议在**反代 / WAF** 层加请求限流与防 CC（注册 IP 限流的后台开关目前仅存配置、尚未在应用层执行）。
