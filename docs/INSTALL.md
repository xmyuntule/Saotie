...
全新部署数据库为空、没有任何用户。两种方式任选其一：

**方式 A：首启自动建（推荐，免手 SQL）** —— 在环境变量里加这两项后首次启动，库里若还没有任何管理员就自动创建该账号（已有管理员则忽略，幂等安全）：
...
启动后用该账号登录即可进后台；**登录后请尽快改密**。建好后这两个变量可去掉。

**方式 B：先注册再提权** —— 先在站点正常注册第一个账号，再把它的角色改成管理员：
