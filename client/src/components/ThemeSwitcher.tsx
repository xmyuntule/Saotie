...
export default function ThemeSwitcher() {
  ...
  return (
    <div className="theme-switch" ref={ref} onBlur={(e) => { if (!e.currentTarget.contains(e.relatedTarget)) setOpen(false); }}
      onKeyDown={(e) => { if (e.key === 'Escape' && open) { e.preventDefault(); setOpen(false); triggerRef.current?.focus(); } }}>
      <button ref={triggerRef} className="nav-icon-btn" onClick={() => setOpen((o) => !o)} aria-label="主题与配色" title="主题与配色" aria-expanded={open} aria-controls="appearance-panel">
        <Icon name="palette" size={20} />
      </button>
      {open && (
        <div id="appearance-panel" className="ui-card theme-pop">
          <div className="ts-title">外观模式</div>
          <div className="ts-modes">
            <button aria-pressed={theme === 'light'} className={`ts-mode${theme === 'light' ? ' on' : ''}`} onClick={() => theme !== 'light' && toggle(transitionOrigin())}>
              <Icon name="sun" size={15} /> 浅色
            </button>
            <button aria-pressed={theme === 'dark'} className={`ts-mode${theme === 'dark' ? ' on' : ''}`} onClick={() => theme !== 'dark' && toggle(transitionOrigin())}>
