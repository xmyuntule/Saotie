...
export default function RichBody({ text, compact = false }: { text?: string; compact?: boolean }) {
  const t = text || '';
  ...
  return (
    <div className="rich">
      {blocks.map((b, i) => {
        if (compact && (b.t === 'h1' || b.t === 'h2' || b.t === 'h3')) return <p key={i} className="rich-p">{renderInline(b.items[0])}</p>;
