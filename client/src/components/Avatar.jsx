...
export default function Avatar({ user, size = 44, to, ring = false, showV = false, emoji: rawEmoji }) {
  ...
  const style = { width: size, height: size, fontSize: Math.round(size * (a.initial ? 0.42 : 0.5)) };
  ...
  const inner = (
    <span className={`avatar-wrap${frame ? ` has-frame frame-${frame}` : ''}`} style={{ width: size, height: size }}>
      ...
      {showV && user?.verified && <span className="avatar-v" title={user.verifiedNote || '认证用户'}>✓</span>}
    </span>
  );
