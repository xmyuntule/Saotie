...
function emojiBio(bio) {
  if (bio?.startsWith('emoji:')) return '';
  return bio || '';
}

export default function PostCard({ post: initial, onDelete, defaultOpenComments = false }) {
  ...
  const [rewardAmt, setRewardAmt] = useState(18);
  ...
  const like = async () => {
    ...
    catch (e) { setLiked(liked); setLikeCount(likeCount); toast.err(e.message); }
  };
  ...
  const unlock = async () => {
    ...
    try {
      ...
      if (data.bypass) setPost((p) => ({ ...p, content: data.content, media: data.media, locked: null, unlocked: true }));
  ...
  const doReward = async () => {
    ...
    try {
      ...
      patchUser({ points: user.points - amt });
      setRewardOpen(false);
      toast.ok(`已打赏 ${amt} 积分 🎁`);
    } catch (e) { toast.err(e.message); }
  };
  ...
  const bookmark = async () => {
    ...
    catch (e) { setBookmarked(bookmarked); toast.err(e.message); }
  };
  ...
  const pin = async () => {
    setMenuOpen(false);
    try { const { data } = await api.post(`/posts/${post.id}/pin`); setPost((p) => ({ ...p, pinned: data.pinned })); toast.ok(data.pinned ? '已置顶到主页' : '已取消置顶'); }
    catch (e) { toast.err(e.message); }
  };

  const globalPin = async () => {
    setMenuOpen(false);
    try { const { data } = await api.post(`/posts/${post.id}/global-pin`); setPost((p) => ({ ...p, globalPinned: data.globalPinned })); toast.ok(data.globalPinned ? '已全站置顶 24 小时 🚀' : '已取消全站置顶'); }
    catch (e) { toast.err(e.message); }
  };
    ...
    <article className="card post rise">
        ...
        <div className="repost" role="link" tabIndex={0}
          ...
          {post.shared.media?.length > 0 && (
            <div className="repost-media">
              {post.shared.media.slice(0, 3).map((m, i) => (
                m.type === 'image'
      ...
      {shown && (
        <div className="post-body">
          <RichText text={shown} />
          {long && !expanded && <> … <span className="post-fulltext" onClick={() => setExpanded(true)}>查看全文</span></>}
        </div>
      )}
      ...
      <div className="post-actions">
        <button className={`act like${liked ? ' on' : ''}`} onClick={like}>
          <Icon name="heart" size={18} fill={liked} className="ico" /> {likeCount > 0 ? fmtNum(likeCount) : '点赞'}
        </button>
        <button className="act comment" onClick={() => setShowComments((s) => !s)}>
