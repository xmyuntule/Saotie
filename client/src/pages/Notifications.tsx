...
const ICONS: Record<string, string> = { like: 'heart', follow: 'user', comment: 'comment', reply: 'comment', mention: 'at', reward: 'gift', share: 'share', answer: 'help', accept: 'check', redpacket: 'redpacket', event: 'ticket', system: 'bell' };
const NCOLOR: Record<string, string> = { like: 'var(--like)', follow: 'var(--brand)', comment: 'var(--good)', reply: 'var(--good)', mention: 'var(--ink-2)', reward: 'var(--gold-deep)', share: 'var(--ink-2)', answer: 'var(--brand)', accept: 'var(--good)', redpacket: 'var(--gold-deep)', event: 'var(--brand)', system: 'var(--ink-2)' };
const VERB: Record<string, string> = {
  like: '赞了你', follow: '关注了你', comment: '评论了你的动态', reply: '回复了你',
  mention: '提到了你', reward: '打赏了你', share: '转发了你的动态',
  answer: '回答了你的提问', accept: '采纳了你的回答',
  redpacket: '抢了你的红包', event: '报名了你的活动', system: '',
};
          ...
          shown.map((n, i) => {
            ...
            const body = (
              <div className={`notif-row${n.read ? '' : ' unread'}`} onClick={() => open(n)} style={{ cursor: to ? 'pointer' : 'default' }}
                ...
                onKeyDown={to ? (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); open(n); } } : undefined}>
