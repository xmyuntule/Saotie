...
export default function SharePage() {
  ...
  const [content, setContent] = useState(() =>
    sourceUrl ? defaultContent(typeLabel, title, summary, sourceUrl) : "",
  );
  ...
  const doLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErr(e.message);
      if (captcha.required || String(e.message || "").includes("验证码"))
        loadLoginCaptcha();
    } finally {
      setLoginBusy(false);
    }
  ...
  const publish = async () => {
    ...
    try {
      ...
      notifyHost("success", {
        postId: data.post?.id,
        url: `${window.location.origin}/post/${data.post?.id}`,
      });
    } catch (e: any) {
      setErr(e.message);
    } finally {
          ...
          <>
            ...
            <section className="share-editor">
              <div className="share-user-line">
                {user ? (
                  <Avatar user={user} size={44} showV />
                ) : (
                  <span className="share-user-placeholder">
                    <Icon name="user" size={22} />
                  </span>
                )}
                <div className="grow">
                  <div className="share-user-name">
                    {user ? user.nickname : "登录后发布到 Saotie 动态"}
                  </div>
              ...
              <div className="share-publish-row">
                ...
                <button
                  ...
                  type="button"
                  onClick={publish}
                  disabled={busy || !sourceUrl || !content.trim()}
                >
                  {busy ? "发布中..." : "发布"}
                </button>
              </div>
