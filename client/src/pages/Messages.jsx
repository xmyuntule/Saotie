...
export default function Messages() {
  ...
  const [active, setActive] = useState(null); // { peer, messages }
  ...
  const deleteConvo = async (e, peerId) => {
    e.stopPropagation();
  ...
  const setPref = async (peerId, patch) => {
    setMenuFor(null);
    try {
          ...
          {!active ? (
            ...
            <>
              ...
              <div className="chat-msgs">
                {(() => { const lastMine = active.messages.map((m) => m.sender_id).lastIndexOf(user.id); return active.messages.map((m, i) => {
